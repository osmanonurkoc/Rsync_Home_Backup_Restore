<#
    .SYNOPSIS
    Home Backup & Restore Tool (v10.0 - GitHub Release)
    A high-performance, incremental backup utility utilizing Windows Hard Links and .NET File System APIs.

    .DESCRIPTION
    Key Features:
    - Incremental Backups: Uses Kernel32 Hard Links to save disk space (Time Machine style).
    - High-Performance Restore: Utilizes System.IO APIs for maximum throughput.
    - Restore Options: Supports restoring to original location or a custom directory.
    - Smart Filtering: Ignored lists via JSON, system file filtering, and partial download protection.
...
try {
    ...
    # MARK: - Kernel32 P/Invoke (For Hard Links)
    $MethodDefinition = @'
    ...
    <Border Name="MainBorder" Background="{DynamicResource BgBrush}" CornerRadius="8" BorderThickness="1" BorderBrush="{DynamicResource BorderBrush}">
        <Grid>
            ...
            <Grid Name="PanelRestore" Grid.Row="3" Margin="30,20,30,10" Visibility="Collapsed">
                ...
                <TextBlock Text="Select a backup version to restore:" Foreground="{DynamicResource SubTextBrush}" Margin="0,0,0,10" FontSize="14"/>
                ...
                <Button Name="BtnStartRestore" Grid.Row="2" Content="START RESTORE..." Style="{StaticResource ActionBtn}" Height="45" Background="{DynamicResource SurfaceBrush}" BorderBrush="{DynamicResource AccentBrush}" BorderThickness="1" Margin="0,5,0,0"/>
            </Grid>
            ...
            <Grid Name="OverlayRename" Grid.RowSpan="5" Background="{DynamicResource OverlayBrush}" Visibility="Collapsed">
                <Border Width="300" Height="160" Background="{DynamicResource BgBrush}" CornerRadius="8" BorderBrush="{DynamicResource BorderBrush}" BorderThickness="1">
                    <Grid Margin="20">
                        ...
                        <TextBlock Text="Rename Backup" Foreground="{DynamicResource TextBrush}" FontWeight="Bold" FontSize="16"/>
            ...
            <Grid Name="OverlayDelete" Grid.RowSpan="5" Background="{DynamicResource OverlayBrush}" Visibility="Collapsed">
                <Border Width="320" Height="160" Background="{DynamicResource BgBrush}" CornerRadius="8" BorderBrush="{DynamicResource BorderBrush}" BorderThickness="1">
                    <Grid Margin="20">
                        ...
                        <TextBlock Text="Are you sure you want to permanently delete this backup folder?" Grid.Row="1" VerticalAlignment="Center" Foreground="{DynamicResource SubTextBrush}" TextWrapping="Wrap" FontSize="13"/>
                        <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Right">
                ...
                <Border Width="400" Height="220" Background="{DynamicResource BgBrush}" CornerRadius="8" BorderBrush="{DynamicResource BorderBrush}" BorderThickness="1">
                    <Grid Margin="20">
                        ...
                        <StackPanel Grid.Row="2" VerticalAlignment="Center" Margin="0,10">
                            <Button Name="BtnRestoreOriginal" Content="Original Location (C:\Users\...)" Height="40" Margin="0,0,0,10" Style="{StaticResource ActionBtn}"
                                    ToolTip="Restores files to their original location (Desktop, Documents, etc.). Existing files will be overwritten."/>

                            <Button Name="BtnRestoreCustom" Content="Choose Custom Folder..." Height="40" Style="{StaticResource SecondaryBtn}"
                                    ToolTip="Select a specific folder to extract the backup."/>
                        </StackPanel>
...
# MARK: - Embedded Icon
# (Add Base64 String here for the icon)
...
$BtnStartBackup.Add_Click({
    ...
    # --- PHASE 1: SCANNING (Modified to capture empty folders) ---
    ...
    $FoldersToCreate = New-Object System.Collections.Generic.List[String] # [NEW]: List for folders
    $Stack = New-Object System.Collections.Generic.Stack[String]

    foreach ($SrcDir in $SelectedSourceDirs) {
        if (Test-Path $SrcDir) {
            $Stack.Push($SrcDir)
            $FoldersToCreate.Add($SrcDir) # Add root folders too
        }
    }
    ...
    while ($Stack.Count -gt 0) {
        ...
        try {
            ...
            foreach ($Item in $Items) {
                # --- GLOBAL CHECKS ---
                # Check Excluded System/Global Folders first to avoid entering them
                if ($Item.PSIsContainer) {
                     if ($Item.Name -in $ExcludeData.IgnoredSystemFolders) { continue }
                     if ($Item.Name -in $ExcludeData.GlobalFolders) { continue } # e.g. .git, node_modules, build

                     # Check if specific folder is ignored (full path check)
                     ...
                     # If passed checks, add to Stack AND Creation List
                    ...
                    if ($File.Name -like "*.megaignore" -or
                        $File.Name -like "*.part" -or
                        $File.Name -like "*.parts" -or
                        $File.Name -like "*.partial" -or
                        $File.Name -like "*.crdownload" -or
                        $File.Name -like "*.tmp" -or
                        $File.Name -like "~$*") { continue }
    ...
    # If no files found, but folders exist, we proceed to create empty structure.
    # Only stop if BOTH lists are empty.
    ...
    # --- PHASE 2: CREATE DIRECTORY STRUCTURE (Includes Empty Folders) ---
    ...
    # Locate Previous Backup for Hard Links
    $LastBackupDir = $null
    ...
    try {
        foreach ($File in $FilesToCopy) {
            # Cancel Check
            if ($Script:CancelRequest) {
                $TxtStatus.Text = "Cancelling... Cleaning up..."
                [System.Windows.Forms.Application]::DoEvents()
            ...
            try {
                ...
                # No need to create directory here anymore, Phase 2 handled it.
                ...
                # Incremental Check (Time Machine)
                if ($LastBackupDir) {
...
$BtnDeleteConfirm.Add_Click({
    if ($ListBackups.SelectedItem) {
        ...
        try {
            # Force GC to release any file locks held by the script
            [System.GC]::Collect()
            [System.GC]::WaitForPendingFinalizers()

            # Attempt deletion using .NET API
            ...
            $TxtStatus.Text = "Backup permanently deleted."

        } catch {
            # Retry mechanism for stubborn locks (Wait 500ms)
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Could not delete the folder. Please ensure it is not open in Windows Explorer.`n`nError: $($_.Exception.Message)", "File in Use", "OK", "Error")
            }
        }
...
$BtnRenameSave.Add_Click({
    ...
    if (-not [string]::IsNullOrWhiteSpace($New) -and $Old -ne $New) {
        try {
            ...
            $TxtStatus.Text = "Renamed successfully."
        } catch { $TxtStatus.Text = "Rename Failed." }
    } else { $OverlayRename.Visibility = "Collapsed" }
})

# --- Restore Button (Triggers Overlay) ---
$BtnStartRestore.Add_Click({
    if ($ListBackups.SelectedItem -ne $null) {
        # Show options overlay
        $OverlayRestoreMode.Visibility = "Visible"
    } else {
        $TxtStatus.Text = "Please select a backup first."
    }
})

$BtnRestoreCancel.Add_Click({
    $OverlayRestoreMode.Visibility = "Collapsed"
})

# --- Restore: Original Location Option ---
$BtnRestoreOriginal.Add_Click({
    $OverlayRestoreMode.Visibility = "Collapsed"
    Run-RestoreProcess -DestinationPath $UserProfile -IsOriginalLocation $true
})

# --- Restore: Custom Folder Option ---
$BtnRestoreCustom.Add_Click({
    ...
    if ($FolderBrowser.ShowDialog() -eq "OK") {
        ...
        $OverlayRestoreMode.Visibility = "Collapsed"
        Run-RestoreProcess -DestinationPath $CustomPath -IsOriginalLocation $false
    }
})
...
function Run-RestoreProcess {
    param (
        [string]$DestinationPath,
        [bool]$IsOriginalLocation
    )

    # --- Cancel Check for Running Process ---
    if ($Script:IsRestoring) {
        $Script:CancelRequest = $true
        $BtnStartRestore.Content = "CANCELLING..."
    ...
    # Update Main Button to Act as Cancel
    ...
    $TxtStatus.Text = "Analyzing Restore Data..."
    [System.Windows.Forms.Application]::DoEvents()

    try {
        $RestoreItems = Get-ChildItem -Path $Source -Recurse
        $PbStatus.Maximum = $RestoreItems.Count
        $Count = 0
        $CachedRestoreDir = ""
        ...
        # Check if destination is empty (for cleanup logic)
        $IsDestEmpty = (Get-ChildItem -Path $DestinationPath).Count -eq 0
        ...
        foreach ($Item in $RestoreItems) {
            ...
            if ($Script:CancelRequest) {
                ...
                # SAFETY: Only delete custom folder if it was NOT the User Profile AND was empty/created by us
                if (-not $IsOriginalLocation -and $IsDestEmpty) {
                    try {
                        if ([System.IO.Directory]::Exists($DestinationPath)) {
                            [System.IO.Directory]::Delete($DestinationPath, $true)
                        }
                    } catch {}
                    $TxtStatus.Text = "Restore Cancelled (Cleaned up)."
                } else {
                    $TxtStatus.Text = "Restore Cancelled (Files preserved)."
                }

                # Reset UI state
                $Script:IsRestoring = $false
                $BtnStartRestore.Content = "START RESTORE..."
            ...
            if ($Count % 2500 -eq 0) {
                ...
                $TxtStatus.Text = "Restoring... ($Count / $($RestoreItems.Count))"
                [System.Windows.Forms.Application]::DoEvents()
            }
            ...
            $TargetItem = Join-Path $DestinationPath $RelPath

            if ($Item.PSIsContainer) {
                if (-not [System.IO.Directory]::Exists($TargetItem)) {
                    [System.IO.Directory]::CreateDirectory($TargetItem) | Out-Null
                }
            } else {
                # Performance: Directory Caching
                $TargetDir = [System.IO.Path]::GetDirectoryName($TargetItem)
                ...
                # Overwrite enabled
                [System.IO.File]::Copy($Item.FullName, $TargetItem, $true)
            }
        }
        ...
        $TxtStatus.Text = "Restore Failed: $($_.Exception.Message)"
    }
